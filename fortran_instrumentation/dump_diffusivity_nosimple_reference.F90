...
!! Isca's DEFAULT diffusivity path: diffusivity_nml do_simple=.false.,
...
!!     dump_diffusivity_nosimple_reference.F90 -o dump_diffusivity_nosimple_reference
!! Run with JSCA_DUMP_DIR set, then convert with read_dumps.py.

program dump_diffusivity_nosimple_reference
  ...
  input_nml_file(1) = '&diffusivity_nml do_simple=.false. do_entrain=.false. /'
  input_nml_file(2) = ' '
  open(newunit=unit, file='input.nml', status='replace')
  write(unit,'(a)') '&diffusivity_nml do_simple=.false. do_entrain=.false. /'
  close(unit)
  ...
  do j = 1, nlat
    do i = 1, nlon
      ...
      do k = 1, nz
        ...
        q(i,j,k) = max(0.018 * sig_f(k)**2, 1.0e-6)   ! moist near sfc, dry aloft
...
end program dump_diffusivity_nosimple_reference
