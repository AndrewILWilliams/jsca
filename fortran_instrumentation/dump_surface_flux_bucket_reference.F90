!! Golden-fixture driver for Isca's bulk surface fluxes (surface_flux.F90), the BUCKET land path: surface_flux_nml do_simple=.true.,
!! use_virtual_temp=.false., old_dtaudv=.true.; land points with the bucket on,
!! bucket_depth swept empty->full across the 0.75*max threshold, + ocean points.
!!
...
!!     dump_surface_flux_bucket_reference.F90 -o dump_surface_flux_bucket_reference
!! Run with JSCA_DUMP_DIR set, then convert with read_dumps.py.

program dump_surface_flux_bucket_reference
  ...
  eps = 287.04 / 461.50
  dt = 720.0
  ! first 30 points: LAND with the bucket, bucket_depth swept 0..2 (crosses
  ! 0.75*max = 1.5); last 10 points: OCEAN (large bucket, never limited).
  land = .false.; seawater = .true.; avail = .true.
  max_bucket_depth_land = 2.0
  depth_change_lh = 0.0; depth_change_conv = 0.0; depth_change_cond = 0.0
  do i = 1, n
    if (i <= 30) then
      land(i) = .true.; seawater(i) = .false.
      bucket_depth(i) = 2.0 * real(i-1) / 29.0     ! 0.0 .. 2.0
    else
      bucket_depth(i) = 1000.0                      ! ocean: effectively unlimited
    end if
  end do
  ...
  call surface_flux(t_atm, q_atm, u_atm, v_atm, p_atm, z_atm, p_surf, t_surf, &
       t_ca, q_surf, .true., bucket_depth, max_bucket_depth_land, &
  ...
  call jsca_dump_1d('sf_land',   merge(1.0, 0.0, land))
  call jsca_dump_1d('sf_bucket_depth', bucket_depth)
  call jsca_dump_1d('sf_depth_change_lh', depth_change_lh)
...
end program dump_surface_flux_bucket_reference
