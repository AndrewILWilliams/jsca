...
selectable via `convection_scheme` (`SIMPLE_BETTS_MILLER`, `FULL_BETTS_MILLER`,
`DRY`, `NONE`).
...
2. **Convection schemes** (`convection_scheme` in `idealized_moist_phys.F90`):
   ...
   `DRY` (Schneider–Walker dry adjustment) and `FULL_BETTS_MILLER`
   (`betts_miller.f90`, distinct from the simple qe scheme; own capecalcnew + LCL
   table) are done. Next: `RAS` (relaxed Arakawa–Schubert, larger).
